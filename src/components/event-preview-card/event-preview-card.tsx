...
const EventPreviewCard = ({ event }: IEventPreviewCard) => {
    return <div className="w-full flex flex-col p-4 shadow-lg rounded-lg">
        <img src={event.eventPreviewImage ? event.eventPreviewImage : noImage} alt="" />
        ...
        <div className='w-full flex gap-x-4 mt-3'>       
            <Icon className='cursor-pointer p-2 bg-blue-400 rounded-full' component={EditOutlined}/>
            <Icon className='cursor-pointer p-2 bg-red-400 rounded-full' component={DeleteOutlined}/>
        </div>
    </div>
}
